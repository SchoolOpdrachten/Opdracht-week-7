...
public sealed class AttractieStepDefinitions
{
    ...
    // Bestaat Niet
    [Given(@"attractie (.*) heeft id nummer (.*)")]
    public async void GivenAttractieBotsautosHeeftIdNummer(string p0, int p1)
    {
        await _databaseData.Context.Attractie.AddAsync(new Attractie { Id = p1, Naam = p0 });
        await _databaseData.Context.SaveChangesAsync();
    }
    [When(@"attractie id nummer (.*) wordt opgevraagd")]
